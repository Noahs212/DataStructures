    ...
    public V put(K key, V value) {
        ...
        if (table[key.hashCode() % table.length] != null) {
            if (table[key.hashCode() % table.length].getKey().equals(key)) {
                oldkey = table[key.hashCode() % table.length].getValue();
            }
        }
        int index = key.hashCode() % table.length;
        Integer saveRemoveSpot = null;
        Integer firstNull = null;
        for (int i = 0; i <= 6; i++) {
            if (table[index] != null && table[index].isRemoved() && saveRemoveSpot == null) {
                saveRemoveSpot = index;
            }
            if (table[index] == null && firstNull == null) {
                firstNull = index;
            }
        ...
        if (saveRemoveSpot != null) {
            table[saveRemoveSpot] = new LinearProbingMapEntry<>(key, value);
            size++;
            return oldkey;
        }
        if (firstNull != null) {
            table[firstNull] = new LinearProbingMapEntry<>(key, value);
            size++;
            return oldkey;
        }
        return oldkey;
    ...
    public V get(K key) {
        ...
        for (int i = 0; i < 6; i++) {
            if (index < table.length && table[index] != null
                    && table[index].getKey().equals(key) && !table[index].isRemoved()) {
